...
def confusion_matrix(rater_a, rater_b, min_rating=None, max_rating=None):
    ...
    """
    assert(T.shape(rater_a) == T.shape(rater_b))
    if min_rating is None:
        min_rating = np.min(rater_a + rater_b)
    if max_rating is None:
        max_rating = np.max(rater_a + rater_b)
...
def quadratic_weighted_kappa(rater_a, rater_b, min_rating=None, max_rating=None):
    ...
    rating
    """
    rater_a = rater_a.astype('int32')
    rater_b = rater_b.astype('int32')
    assert(len(rater_a) == len(rater_b))
    if min_rating is None:
        min_rating = np.min(np.min(rater_a), np.min(rater_b))
    if max_rating is None:
        max_rating = np.max(np.max(rater_a), np.max(rater_b))
    ...
    for i in range(num_ratings):
        for j in range(num_ratings):
            expected_count = (hist_rater_a[i] * hist_rater_b[j]
                              / num_scored_items)
            d = np.power(i - j, 2.) / np.power(num_ratings - 1, 2.)
