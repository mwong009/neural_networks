...
class rmsprop(object):
    ...
    def __init__(self, params):
        self.running_square_ = [theano.shared(
            value=np.zeros_like(p.get_value()),
            name=p.name+'_running_square',
            borrow=True) for p in params]

        self.running_avg_ = [theano.shared(
            value=np.zeros_like(p.get_value()),
            name=p.name+'_running_avg',
            borrow=True) for p in params]

        self.memory_ = [theano.shared(
            value=np.zeros_like(p.get_value()),
            name=p.name+'_memory',
            borrow=True) for p in params]
...
class sgd_nesterov(object):
    def __init__(self, params):
        self.memory_ = [theano.shared(
            np.zeros_like(p.get_value(), dtype=theano.config.floatX),
            name=p.name+'_memory',
            borrow=True) for p in params]
